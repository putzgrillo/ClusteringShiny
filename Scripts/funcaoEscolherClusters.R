...
funcaoEscolherClusters <- function(df, varFixasNm, varCombnNm, percPior = 0.03, intK = 3:12, paralelo = FALSE, nucleos = 3, semente = 15081991) {
  ...
  if (paralelo) {
    library(parallel)
    retornoClusters <- mclapply(combVariaveis, function(x) {
      funcaoCalcularClusters(x = df[, x], kTest = intK, idClust = paste0(x, collapse = "|"), seed = semente)
    }, mc.cores = nucleos)
  } else {
